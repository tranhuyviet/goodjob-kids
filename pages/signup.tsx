        ...
        <div className="container h-screen shadow-md absolute inset-0 bg-blue-200 flex justify-center items-center">
            <div className="text-center w-full flex flex-col justify-center items-center">
                ...
                <form className="mt-2 w-full flex flex-col items-center" onSubmit={handleSubmit} noValidate>
                    <input autoFocus type="text" className="block w-3/4 rounded-2xl shadow-sm focus:border-0 focus:ring-green-600 focus:ring-2 h-[42px] tracking-wider text-center" name="name" value={values.name} onChange={handleChange} />
                    {errors && errors.name && <p className="text-red-500 mt-2">{errors.name}</p>}
                    <button className="py-1 px-7 rounded-xl border shadow-md bg-green-600 text-gray-50 text-lg tracking-wider mt-4" type="submit" >Submit</button>
                </form>
            </div>
        </div>
    )
